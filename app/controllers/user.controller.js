...
exports.findAll = (req, res) => {
  ...
  var timeBegin = isNaN(Number(rQuery.timeBegin))
    ? undefined
    : Number(rQuery.timeBegin);
  var timeEnd = isNaN(Number(rQuery.timeEnd))
    ? undefined
    : Number(rQuery.timeEnd);
  var request = rQuery.request;
  var queryFind = {};

  if (timeBegin || timeEnd) {
    queryFind.time = {};
    if (timeBegin) queryFind.time["$gte"] = timeBegin;
    if (timeEnd) queryFind.time["$lte"] = timeEnd;
  }

  if (request) {
    queryFind.request = new RegExp(request, "i");
  }
  // db.users.find({$text:{$search: "raj",$caseSensitive: false}})
  User.count(queryFind, function(error, count) {
    if (!error) {
      User.find(queryFind, function(err, logItems) {
        if (!err) {
          return res.json({ results: logItems, count: count });
        } else {
